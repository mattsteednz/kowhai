...
- **Progress sync (PRD-39)** — Listening positions are now automatically backed up to `positions.json` in the audiobooks root folder within 30 seconds of any position change, and immediately when the app backgrounds. This file survives app reinstalls as long as the folder is intact. An optional "Sync progress to Drive" toggle in Settings (Drive section) uploads the backup to an `AudioVault/` subfolder in a chosen Drive location; on first launch with an empty database and sync enabled, positions are silently restored from Drive. Drive write access is requested only when the toggle is turned on. Shared Drive folders are not eligible as backup destinations — the app prompts for a writable folder instead.
